...
namespace RoverControlApp.MVVM.Model
{
	public class OnvifPtzCameraController
	{
		...
		public Vector4 CameraMotion
		{
			...
			private set
			{
				if (MainViewModel.Settings.Settings.VerboseDebug) MainViewModel.EventLogger.LogMessege($"PTZ: CameraMotion update: {value}");
				_dataMutex.WaitOne();
				_cameraMotion = value;
				_dataMutex.ReleaseMutex();
			}
		...
		public CommunicationState State
		{
			...
			private set
			{
				MainViewModel.EventLogger.LogMessege($"PTZ: CommunicationState update: {value}");
				_state = value;
			}
		}
		...
		{
			MainViewModel.EventLogger.LogMessege("PTZ: Thread started");
		...
		private void CreateCamera()
		{
			...
			{
				MainViewModel.EventLogger.LogMessege($"PTZ: Connecting to camera failed after " +
			...
			MainViewModel.EventLogger.LogMessege($"PTZ: Connecting to camera succeeded in {(int)_generalPurposeStopwatch.Elapsed.TotalSeconds}s");
		...
		private void DoWork(ref Vector4 motionLast)
		{
			switch (State)
			{
				...
				case CommunicationState.Opened:
					...
					{
						MainViewModel.EventLogger.LogMessege($"PTZ: Handled exception {e} caught");
						errCaught = true;
					}

					if (_generalPurposeStopwatch.Elapsed.TotalSeconds > 10 || errCaught)
					{
						MainViewModel.EventLogger.LogMessege($"PTZ: Camera connection lost ;( Sending a move request took {(int)_generalPurposeStopwatch.Elapsed.TotalSeconds}s");
						State = CommunicationState.Faulted;
						EndCamera();
						return;
					}
